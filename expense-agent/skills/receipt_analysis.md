...
2. **금액 처리:**
   ...
   - **통화 우선순위: KRW(원화)를 최우선으로 사용**
   - KRW와 외화(USD 등)가 함께 있으면 반드시 KRW 금액을 사용
   - KRW가 없고 외화만 있는 경우:
     1. 문서에 환율이 명시되어 있으면 그 환율로 KRW 환산하여 반환
     2. 환율이 없으면 거래일 기준 대략적 환율로 환산하고, summary_inference에 "(환율 추정, 확인 필요)" 문구를 추가
